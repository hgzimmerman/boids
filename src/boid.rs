use pinwheel::{
    prelude::{svg::polygon, Component},
    signal::{Mutable, Signal, SignalExt},
    Node,
};

use crate::config::Config;
use crate::{nearest_neightbor::NearestNeightborsMap, vector::Vec2d};
...
impl Boid {
        ...
        self.inner.replace_with(|boid| {
            ...
            BoidInner {
                ...
                velocity: (boid.velocity + boid.acceleration).clamp(config.max_speed)
                    * (std::cmp::max(12, count) as f32 / 12.0),
                acceleration: {
                    let mut acceleration = if count == 0 {
                        boid.acceleration
                    } else {
                        let coherence_acceleration =
                            -(boid.position - position_sum / count as f32) * config.coherence;
                        let alignment_acceleration =
                            (velocity_sum / count as f32) * config.alignment;
    ...
    fn points2(&self) -> impl Signal<Item = String> {
        ...
        self.inner.signal().map(move |boid| {
            let BoidInner {
                position,
                velocity,
                acceleration: _,
