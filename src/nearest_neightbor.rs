use crate::{boid::BoidInner, vector::Vec2d};
...
impl NearestNeightborsMap {
    pub(crate) fn new(edge_length: i32, boids: impl IntoIterator<Item = BoidInner>) -> Self {
        let map = boids
            .into_iter()
            .fold(Map::default(), |mut map: Map, boid| {
                map.entry(Zone::new(boid.position, edge_length))
                    .or_default()
                    .push(boid);
                map
            });
    ...
    pub(crate) fn find_within_radius(
        &self,
        origin: Vec2d,
        radius: f32,
    ) -> impl Iterator<Item = BoidInner> + '_ {
